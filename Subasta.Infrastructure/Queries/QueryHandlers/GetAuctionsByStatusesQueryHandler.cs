...
{
    public class GetAuctionsByStatusesQueryHandler : IRequestHandler<GetAuctionsByStatusesQuery, List<GetAuctionDto>>
    {
        ...
        public GetAuctionsByStatusesQueryHandler(IReadAuctionRepository auctionReadRepository)
        {
            _auctionReadRepository = auctionReadRepository ?? throw new ArgumentNullException(nameof(auctionReadRepository));
        }

        public async Task<List<GetAuctionDto>> Handle(GetAuctionsByStatusesQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var auctions = await _auctionReadRepository.GetAuctionsByStatuses(request.Statuses);
