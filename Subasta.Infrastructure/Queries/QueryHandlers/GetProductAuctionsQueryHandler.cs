...
{
    public class GetProductAuctionsQueryHandler : IRequestHandler<GetProductAuctionsQuery, List<GetAuctionDto>>
    {
        ...
        public GetProductAuctionsQueryHandler(IReadAuctionRepository auctionReadRepository)
        {
            _auctionReadRepository = auctionReadRepository ?? throw new ArgumentNullException(nameof(auctionReadRepository));
        }

        public async Task<List<GetAuctionDto>> Handle(GetProductAuctionsQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var auctions = await _auctionReadRepository.GetProductAuctions(request.ProductId);
