...
{
    public class GetAuctionsInRangeQueryHandler : IRequestHandler<GetAuctionsInRangeQuery, List<GetAuctionDto>>
    {
        private readonly IReadAuctionRepository _auctionReadRepository;

        public GetAuctionsInRangeQueryHandler(IReadAuctionRepository auctionReadRepository)
        {
            _auctionReadRepository = auctionReadRepository ?? throw new ArgumentNullException(nameof(auctionReadRepository));
        }

        public async Task<List<GetAuctionDto>> Handle(GetAuctionsInRangeQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var auctions = await _auctionReadRepository.GetAuctionsInRange(request.From, request.To);
